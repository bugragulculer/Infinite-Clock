...
function Column({max, current}) {
  return (
    <div
      className='column'
      data-id={current}
      // Style part creates the animation of the current value. Change the below number -50 with the column_height value in App.css file. (make it equal to each other)
      // It takes the current value and multiplies with the -50 value.
      style={{ top: `${-50 * current}px` }}
    >
      {Array(max + 1)
        .fill(<i/>)
        .map((_e, i) => ( 
